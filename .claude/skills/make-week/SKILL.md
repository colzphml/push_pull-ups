...
sqlite3 history/training.db "SELECT date,block,exercise,planned,actual,card_title,done,felt,note FROM exercise_log WHERE week_id=(SELECT MAX(id) FROM training_weeks);"
```
...
2. Залить факт в `exercise_log`. Ключ связи — `date + block + card_title`, где `card_title`
   ...
   Осторожно с одинаковыми именами: поле `exercise` **в JSON-логе** — это как раз текст
   карточки, его значение и подставляется в сравнение с `card_title`. Сопоставлять
   `log.exercise` с `exercise_log.exercise` нельзя, они про разное.
...
3¾. **Произвольные активности** (с недели 11): записи лога с `block = 'custom'` добавлены
   пользователем кнопкой «+ активность» на сайте — плановых строк для них в `exercise_log`
   НЕТ, поэтому не UPDATE, а INSERT:

   ```sql
   INSERT INTO exercise_log (week_id,date,"window",block,exercise,card_title,planned,done,actual,felt,note)
   VALUES (<week_id>,'2026-08-14','своё','custom','🚴 Велосипед','🚴 Велосипед','',1,'40 мин','','');
   ```

   Перед INSERT проверить, что строки с таким `date+block+card_title` ещё нет (разбор мог
   уже запускаться). Записи с `deleted = 1` — tombstone удалённых: пропускать полностью
   (не вставлять; если ранее вставлена — удалить). Custom-активности — сигнал для
   планирования: что человек делает сам, то и стоит встраивать в недели.

   Блок `steps` (карточка «10 000 шагов») — обычные плановые строки, для них UPDATE как всегда.
...
Залитый факт (см. выше) — основа; уточни словами то, что трекер не фиксирует: новые
замеры (вис, отжимания), причины пропусков, самочувствие в целом за неделю.
...
- +1 карточка шагов в день (блок `steps`, окно «За день», card_title «10 000 шагов»,
  planned «цель 10 000») — по запросу colz от 10.08.2026, детали в `profile.json.steps`.
  Шаги остаются и в лёгкие дни (ходьба — не тренировка). В hitDays трекера не входят.

## ШАГ 5 — Создать HTML недели
**Имя:** `weeks/2026/week_YYYY-MM-DD.html` (YYYY-MM-DD = понедельник).
- Скопируй структуру и стили из последней недели (эталон логики — `weeks/2026/week_2026-08-10.html`,
  НЕ изобретай дизайн): тема, точки `dot-pull/dot-push/dot-mob/dot-cold` (циан `#5ec8d8`)
  и `dot-steps` (зелёный `#6cc08b`) + баннер `.steps-banner`, nav (Дни/Прогрессия/Техника),
  флип-модалка, PTR, регистрация SW `/push_pull-ups/sw.js`. Кнопку «+ активность» и карточки
  custom-записей трекер рисует сам — в разметке недели для них ничего не нужно.
...
- Перед `</body>` вставить: `<script type="module" src="../../assets/tracker.js"></script>` — трекер отметок.
