# CLAUDE.md — проект push_pull-ups

## Контекст
Персональная программа отжиманий и подтягиваний с нуля для colz (~130 кг,
снижает вес — см. проект health_food). Методика — Greasing the Groove: лёгкие
подходы вне отказа, разнесённые по дню. Цель — система/привычка и постепенный
прогресс. Старт: отжимания от стены (P0), вис (H0, ~20 сек). Третий трек —
закаливание холодной водой (лестница C0–C7 → купель, см. `plan/cold.md`).
Четвёртый трек (с 21.08.2026) — гиря 12 кг: лестница KB0–KB3, сначала шарнир и
становая, свинг позже (см. `plan/kettlebell.md`).

## Файлы контекста — читать перед генерацией недели
| Файл | Что |
|---|---|
| `profile.json` | 🔒 текущие уровни push/pull, базовые цифры, вес (в .gitignore) |
| `plan/progression.md` | мастер-план: лестницы, milestones, техника, GtG |
| `plan/cold.md` | мастер-план закаливания: лестница C0–C7, красные линии, сезонная карта |
| `plan/kettlebell.md` | мастер-план гири: лестница KB0–KB3, дозировки, красные линии (спина!) |
| `equipment.md` | турник, резинки (только на турник!), опоры |
| `history/training.db` | SQLite: weeks, exercise_log, progress, training_rules |
| `history/log/*.json` | факт выполнения с сайта (трекер) — заливать в `exercise_log` перед генерацией |

## Главный скилл
`/make-week` — генерирует следующую неделю. Запускать через Skill tool.
Перед генерацией ОБЯЗАТЕЛЬНО вычитать активные `training_rules`.

## training_rules (rating)
- `-2` боль/стоп → НИКОГДА не давать (заменить + мобилизация)
- `-1` временно убрать (пока `valid_until >= today`)
- `+1` норм · `+2` «зашло» — держать чаще

## Безопасность (важно при ~130 кг)
- Резинки — ТОЛЬКО петлёй через турник (нога/колено в петле). Двери/мебель как
  якорь НЕ использовать.
- Турник должен держать ≥150 кг.
- Никогда до отказа. Боль = стоп. Болезнь = стоп (тренировки И закалка).
- Закалка: никакой гипервентиляции/задержек дыхания у воды; купель/прорубь —
  никогда в одиночку; выходить до сильной дрожи. Детали — `plan/cold.md`.

## Структура HTML
Эталон: `weeks/2026/week_2026-08-10.html`. Тема — холодная (`--accent:#4f9dd9`,
push `#e0a04e`, mob `#a07fc4`, cold `#5ec8d8`, steps `#6cc08b`). Карточка: лицо +
оборот-модалка (техника). Вкладки Дни/Прогрессия/Техника. Один человек — без
person-switch. SW: `/push_pull-ups/sw.js`.

Каждая неделя подключает трекер отметок: `<script type="module" src="../../assets/tracker.js"></script>`
перед `</body>`. Трекер читает разметку сам (дата из `.day-col[data-date]`, блок из класса
`dot-*`, упражнение из `.meal-title`, план из `.meal-kcal`) — дополнительных атрибутов не нужно.
В `exercise_log.card_title` при генерации писать текст `.meal-title` дословно: это ключ, по
которому отметки находят свою строку.

Блоки трекера: `push/pull/mob` (входят в hitDays), `kb` — гиря (точка `#d97a6c`) — с недели 14
отдельный ЕЖЕДНЕВНЫЙ трек со своим счётчиком «🏋️ гиря», в hitDays НЕ входит (решение colz
31.08.2026: «гиря не вместо отжиманий и подтягиваний»), `cold`, `steps` (карточка «10 000 шагов»,
каждый день), `custom` — произвольные активности, которые пользователь добавляет кнопкой
«+ активность» (кнопку и карточки рисует сам трекер, в разметке недели их нет). Custom-записи
живут в `history/log/*.json` с `done=1`; удалённые несут `deleted:1` (tombstone) — при анализе
пропускать. Для custom в `exercise_log` нет плановых строк — при разборе факта они вставляются
INSERT'ом (см. SKILL.md make-week).

## Деплой
Репо: `https://github.com/colzphml/push_pull-ups.git`
Pages: `https://colzphml.github.io/push_pull-ups/`
```bash
git pull --rebase origin main          # трекер пишет отметки прямо в main — без этого push отобьёт
git add weeks/ index.html history/training.db
git commit -m "Неделя ..." && git push origin main
```
`profile.json` — в .gitignore, не пушить.

## Тесты
Чистая логика трекера покрыта тестами, зависимостей нет:
```bash
node --test tests/*.test.js            # маска обязательна: `node --test tests/` в Node 26 не работает
```
